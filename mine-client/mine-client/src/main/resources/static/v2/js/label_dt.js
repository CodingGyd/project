$(document).ready(function() {
	...
	page = Request['page'];
	limit = Request['limit'];
	label_dm = Request['label_dm'];
	label_ms = Request['label_ms'];

	if (label_dm == 'undefined') label_dm = null;
	
	loadList(page,limit,label_dm);
});

function loadList(page,limit,label_dm){
 	 $.ajax({
	        type: "Post",
	        url: "/article_page?page="+page+"&limit="+limit+"&label_dm="+label_dm+"&label_ms="+label_ms,
	        async:false,
	        success: function(result){
	        	
	        	if (null != result && null != result.keyWordRelation) {
	        		$("#label_ms").text(result.keyWordRelation.keyName);
	        	}
	        	if (null != result && null != result.data) {
	        		...
	        		content += '&nbsp;&nbsp;&nbsp;<b>'+cur+'</b>&nbsp;';
	          		if (null != next) {
	        			content += '<a href="/label_dt?page='+next+'&limit=10&label_dm='+label_dm+'"&label_ms="'+label_ms+'">'+next+'</a>&nbsp;';
	        			content += '<a href="/label_dt?page='+next+'&limit=10&label_dm='+label_dm+'"&label_ms="'+label_ms+'">下一页</a>&nbsp;';
	        			content += '<a href="/label_dt?page='+end+'&limit=10&label_dm='+label_dm+'"&label_ms="'+label_ms+'">尾页</a>&nbsp;';
	        		}
	          		
	        		$('#dv_label_dt_pagelist').append(content);
