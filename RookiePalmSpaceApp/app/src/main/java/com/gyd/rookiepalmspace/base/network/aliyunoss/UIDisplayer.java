...
public class UIDisplayer {
    ...
    private static final String TAG = "UIDisplayer";
        ...
        handler = new Handler(Looper.getMainLooper()) {
            ...
            public void handleMessage(Message inputMessage) {
                ...
                switch (inputMessage.what) {
                    case UPLOAD_OK:
                        if (!UIDisplayer.this.activity.isFinishing()) {
                            new AlertDialog.Builder(UIDisplayer.this.activity).setTitle("提示").setMessage("上传成功!").show();
                        }else {
                            ToastUtil.show(UIDisplayer.this.activity, "上传成功!");
                        }
                        break;
                    case UPLOAD_FAIL:
                        info = (String) inputMessage.obj;
                        LogUtil.e(TAG, "UIDisplayer.this." + UIDisplayer.this.activity.isFinishing() + "");
                        if (!UIDisplayer.this.activity.isFinishing()) {
                            new AlertDialog.Builder(UIDisplayer.this.activity).setTitle("上传失败").setMessage(info).show();
                        } else {
                            ToastUtil.show(UIDisplayer.this.activity, "上传失败");
                        }
                        break;
                    case DOWNLOAD_OK:
                        LogUtil.e(TAG, "UIDisplayer.this." + UIDisplayer.this.activity.isFinishing() + "");
                        if (!UIDisplayer.this.activity.isFinishing()) {
                            ...
                            ToastUtil.show(UIDisplayer.this.activity, "下载成功");
                        }
                        break;
                    case SETTING_OK:
                        LogUtil.e(TAG, "UIDisplayer.this." + UIDisplayer.this.activity.isFinishing() + "");
                        if (!UIDisplayer.this.activity.isFinishing()) {
                            ...
                            ToastUtil.show(UIDisplayer.this.activity, "设置域名信息成功,现在<选择图片>, 然后上传图片");
                        }
                        break;
                    case DOWNLOAD_FAIL:
                        LogUtil.e(TAG, "UIDisplayer.this." + UIDisplayer.this.activity.isFinishing() + "");
                        ...
                        if (!UIDisplayer.this.activity.isFinishing()) {
                            ...
                            ToastUtil.show(UIDisplayer.this.activity, "下载失败");
                        }
                        break;
