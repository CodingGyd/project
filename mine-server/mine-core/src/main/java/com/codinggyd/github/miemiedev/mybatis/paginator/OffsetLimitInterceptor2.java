...
public class OffsetLimitInterceptor2 implements Interceptor {
	private static Logger logger = LoggerFactory.getLogger(OffsetLimitInterceptor2.class);
	...
	public Object intercept(final Invocation invocation) throws Throwable {
		...
		if (rowBounds == null || rowBounds == RowBounds.DEFAULT) {
			...
			if (end - start > 2000) {
				Log.error(logger, "MS_ID:[{}],参数:[{}],执行时间:[{}]", ms.getId(), queryArgs,end - start);
			}
			return result;
		}
...
//			return invocation.proceed();
		...
		queryArgs[ROWBOUNDS_INDEX] = RowBounds.DEFAULT;// new
	...
	public static MappedStatement processMappedStatement(MappedStatement ms) throws Throwable {
		...
		// pageSqlSource = new PageStaticSqlSource((StaticSqlSource)
		// tempSqlSource, parser);
		...
		// pageSqlSource = new PageProviderSqlSource((ProviderSqlSource)
		// tempSqlSource, parser);
		// } else if (tempSqlSource instanceof DynamicSqlSource) {
		// pageSqlSource = new PageDynamicSqlSource((DynamicSqlSource)
		// tempSqlSource, parser);
		// } else {
